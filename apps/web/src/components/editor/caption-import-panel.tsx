...
export function CaptionImportPanel({ onImportSuccess }: CaptionImportPanelProps) {
    ...
    lines.forEach((line, index) => {
      ...
      if (parts.length === 3) {
        ...
        if (!isNaN(startTime) && !isNaN(endTime) && text) {
          ...
          toast.error(`Skipped invalid line in CSV: ${line.substring(0, 50)}...`);
        }
      } else {
        console.warn(`Skipping malformed CSV line ${index + 1}: ${line}`);
        toast.error(`Skipped malformed line in CSV: ${line.substring(0, 50)}...`);
      }
    });
    return captions;
  };
      ...
      if (lines.length >= 3) {
        ...
        try {
          ...
          if (!isNaN(startTime) && !isNaN(endTime) && text) {
            ...
            toast.error(`Skipped invalid SRT block: ${block.substring(0, 50)}...`);
          }
        } catch (error) {
          console.error(`Error parsing SRT timecode in block ${index + 1}: ${timecodeLine}`, error);
          toast.error(`Failed to parse timecode in SRT block: ${timecodeLine.substring(0, 50)}...`);
        }
      } else {
        console.warn(`Skipping malformed SRT block ${index + 1}: ${block.substring(0, 100)}...`);
        toast.error(`Skipped malformed SRT block: ${block.substring(0, 50)}...`);
      }
    });
    return captions;
  };
      ...
      try {
        switch (fileExtension) {
          ...
          case 'srt':
            ...
            break;
          default:
            toast.error("Please upload a .txt, .csv, or .srt file.");
            return;
        }
        ...
        if (parsed.length > 0) {
          ...
          toast.error("The selected file did not contain any valid captions.");
        }
      } catch (error) {
  ...
  const handleConfirmImport = () => {
    ...
    toast.success(`${parsedCaptions.length} captions have been successfully imported.`);
    onImportSuccess?.();
  };
  ...
  const handleClearPreview = () => {
    setParsedCaptions([]);
    setIsModalOpen(false);
    toast.success("Caption import has been cancelled.");
  };
